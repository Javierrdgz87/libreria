...
@section('content')
	...
	<div class="container">
		...
		<form method="POST" action="{{ route('categorias.update', $aDatos->eCodCategoria) }}">
			{!! method_field('PUT') !!}
			{!! csrf_field() !!}
			     	...
			     	<div class="row">
				      	...
				      	<div class="col-md-4">
				      		<div class="form-group">
							    ...
							    <input type="text" class="form-control" id="tDescripcion" name="tDescripcion" placeholder="Descripci&oacute;n" value="{{ $aDatos->tDescripcion }}">
							</div>
		      			</div>
		      		</div>
		      		@if(0 < count($aDatos->libros) )
			      		<div class="row">
			      			<div class="col-md-4">
			      				<label for="">Libros relacionados</label>
			      				<ul>
				      			@foreach($aDatos->libros as $value)
									<li>{{ $value->tNombre }}</li>
				      			@endforeach
				      			</ul>
